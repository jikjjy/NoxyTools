...
{
    public class RegionEditor : RegionEditorWindow
    {
        ...
        private RegionSet mSelectItem = new RegionSet();
        private DataTable mDataTable = new DataTable();

        public RegionEditor() : base()
        {
            initializeComponent();
            registEvent();
        }
        ...
        private void initializeComponent()
        {
            ...
            mDataTable.Columns.Add(GridColumn.RAWDATA, typeof(RegionSet));
        ...
        private void registEvent()
        {
            ...
            txbLocations.Click += txbLocations_Click;
            txbCreeps.Click += txbCreeps_Click;
        ...
        private void grdItemGrade_SelectionChanged(object sender, EventArgs e)
        {
            ...
            {
                mSelectItem = (grdList.SelectedRows[0].Cells[GridColumn.RAWDATA].Value as RegionSet).DeepClone();
            }
            updateEditUi();
        }
        ...
        private void txbLocations_Click(object sender, EventArgs e)
        {
            ListEditor<LocationSet>.ShowDialog(mNoxpedia.Data.Locations, mSelectItem.Locations, "장소 리스트 편집", string.Empty);
            updateEditUi();
        }

        private void txbCreeps_Click(object sender, EventArgs e)
        {
            ListEditor<CreepSet>.ShowDialog(mNoxpedia.Data.Creeps, mSelectItem.Creeps, "크리쳐 리스트 편집", string.Empty);
            updateEditUi();
        }
        ...
        {
            if (mNoxpedia.Data.ModifyRegion(mSelectItem) == false)
            {
                txbName.Focus();
                txbName.SelectAll();
        ...
        {
            if (mNoxpedia.Data.Regions.Any(item => item.Name == mSelectItem.Name) == false)
            {
            ...
            mNoxpedia.Data.DeleteRegion(mSelectItem);
            updateList();
            mNoxpedia.RasieDataChangedEvent();
        }
        ...
        {
            if (mNoxpedia.Data.AddRegion(mSelectItem) == false)
            {
                txbName.Focus();
                txbName.SelectAll();
        ...
        private void commandRenameItem()
        {
            ...
            if (mNoxpedia.Data.Regions.Any(item => item.Name == mSelectItem.Name) == true)
            {
        ...
        private void updateEditUi()
        {
            ...
            foreach (var item in mSelectItem.Locations)
            {
                sb.AppendLine($"[{item.Name}]");
            }
            txbLocations.Text = sb.ToString();
            sb.Clear();
            foreach (var item in mSelectItem.Creeps)
            {
                sb.AppendLine($"[{item.Name}]");
            }
            txbCreeps.Text = sb.ToString();

            bool existItem = mNoxpedia.Data.Regions.Any(item => item.Name == mSelectItem.Name);
        ...
        private void updateList(string recoveryItemName = "")
        {
            ...
            foreach (var item in mNoxpedia.Data.Regions)
            {
